...
App.router = {
    ...
    resolve() {
        const hashRaw = window.location.hash.slice(1) || '/login';
        const [hash, queryString] = hashRaw.split('?');
        const { path, params } = this.matchRoute(hash);

        const queryObj = {};
        if (queryString) {
            new URLSearchParams(queryString).forEach((val, key) => { queryObj[key] = val; });
        }
        ...
        this.renderView(viewName, params, path, queryObj);
    },
    ...
    async renderView(viewName, params, path, queryObj = {}) {
