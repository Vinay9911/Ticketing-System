...
const assets = {
    ...
    async getRaw(id) {
        return queryOne('SELECT * FROM ts_assets WHERE id = $1 AND a.is_deleted = FALSE', [id]);
    },
    ...
    async create({ name, category_id, serial_number, express_service_code, make_model, purchase_date, cost, warranty_start_date, warranty_expiry, location, status, notes, created_by }) {
        ...
        `, [name, category_id || null, serial_number || null, express_service_code || null, make_model || null, purchase_date || null, cost || null,
            warranty_start_date || null, warranty_expiry || null, location || null, status || 'available', notes || null, created_by]);
    },
    ...
    async update(id, { name, category_id, serial_number, express_service_code, make_model, purchase_date, cost, warranty_start_date, warranty_expiry, location, status, notes }) {
        await run(`
            UPDATE ts_assets SET name=$1, category_id=$2, serial_number=$3, express_service_code=$4, make_model=$5, purchase_date=$6, cost=$7,
            warranty_start_date=$8, warranty_expiry=$9, location=$10, status=$11, notes=$12, updated_at=NOW()
            WHERE id=$13
    ...
    async assign(id, { assigned_to_user, assigned_to_dept }) {
        return run(`UPDATE ts_assets SET assigned_to_user=$1, assigned_to_dept=$2, status='in_use', updated_at=NOW() WHERE id=$3`,
            [assigned_to_user || null, assigned_to_dept || null, id]);
    },

    async unassign(id) {
        return run(`UPDATE ts_assets SET assigned_to_user=NULL, assigned_to_dept=NULL, status='available', updated_at=NOW() WHERE id=$1`, [id]);
    },
...
const categories = {
    ...
    async create({ name, description }) {
        return insertReturning('INSERT INTO ts_asset_categories (name, description) VALUES ($1, $2)', [name, description || null]);
    },
...
const tickets = {
    ...
    async getById(id) {
        return queryOne(`
            SELECT t.*, ru.name AS raised_by_name, ru.role AS raised_by_role, ru.email AS raised_by_email, ru.emp_id AS raised_by_emp_id, d.name AS raised_by_dept,
    ...
    async update(id, updates) {
        ...
        if (updates.priority) { setClauses.push(`priority = $${idx++}`); params.push(updates.priority); }
        if (updates.title) { setClauses.push(`title = $${idx++}`); params.push(updates.title); }
        if (updates.description) { setClauses.push(`description = $${idx++}`); params.push(updates.description); }
        if (updates.issue_type) { setClauses.push(`issue_type = $${idx++}`); params.push(updates.issue_type); }
    ...
    async assign(id, assignedTo) {
        return run("UPDATE ts_tickets SET assigned_to = $1, status = 'in_progress', updated_at = NOW() WHERE id = $2", [assignedTo, id]);
    },
    ...
    async getStats({ role, userId, departmentId }) {
        ...
        const byPriority = await queryAll(`
            SELECT priority, COUNT(*) AS count FROM ts_tickets WHERE 1=1 ${deptFilter} GROUP BY priority
        `, params);
        const byStatus = await queryAll(`
            SELECT status, COUNT(*) AS count FROM ts_tickets WHERE 1=1 ${deptFilter} GROUP BY status
        `, params);
        ...
        return {
            ...
            avgResolutionHours: avgRes.avg_hours ? Math.round(parseFloat(avgRes.avg_hours)) : null
        };
    }
};
...
const repairs = {
    async getAll({ conditions, params, limit, offset }) {
        const where = conditions.join(' AND ');
        const countRes = await queryOne(`SELECT COUNT(*) AS c FROM ts_repairs r WHERE \${where}`, params);
        const total = parseInt(countRes.c);
    ...
    async getById(id) {
        return queryOne('SELECT * FROM ts_repairs WHERE id = $1', [id]);
    },
    ...
    async create({ asset_id, repair_date, issue_description, assigned_to, requires_approval, cost, invoice_path, is_approved, status, provider, notes, created_by }) {
        ...
        `, [asset_id, repair_date, issue_description, assigned_to || null, requires_approval || false, is_approved || false, status || 'pending', invoice_path || null, provider || null, cost || null, notes || null, created_by]);
    },
    ...
    async getUpcoming() {
        return queryAll(`
            SELECT r.*, a.name AS asset_name, u.name AS assigned_to_name
            FROM ts_repairs r
            LEFT JOIN ts_assets a ON r.asset_id = a.id
            LEFT JOIN ts_users u ON r.assigned_to = u.id
    ...
    async getStats() {
        ...
        `)).c);
        return { pending, inProgress, completed, warrantyExpiring };
    }
};
...
const notifications = {
    ...
    async notifyDeptManagers(departmentId, type, title, message, referenceId) {
        const managers = await queryAll("SELECT id FROM ts_users WHERE role = 'manager' AND department_id = $1 AND is_active = TRUE", [departmentId]);
...
const reports = {
    ...
    // Export-specific data queries
    async exportAssetData() {
        return queryAll(`SELECT a.name, c.name AS category, a.serial_number, a.status, a.cost,
            a.location, u.name AS assigned_to FROM ts_assets a
            LEFT JOIN ts_asset_categories c ON a.category_id = c.id
            LEFT JOIN ts_users u ON a.assigned_to_user = u.id WHERE a.is_deleted = FALSE`);
    },

    async exportTicketData() {
        return queryAll(`SELECT t.ticket_number, t.title, t.priority, t.status, t.issue_type,
            u.name AS raised_by, t.created_at FROM ts_tickets t
            LEFT JOIN ts_users u ON t.raised_by = u.id`);
    },

    async exportRepairData() {
        return queryAll(`SELECT a.name AS asset_name, r.issue_description, r.repair_date, r.status,
            u.name AS technician, r.completed_at FROM ts_repairs r
            LEFT JOIN ts_assets a ON r.asset_id = a.id LEFT JOIN ts_users u ON r.assigned_to = u.id`);
    },
